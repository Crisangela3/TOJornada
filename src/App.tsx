...
const fallbackDatabase: Record<number, { perguntas: { facil: { q: string; a: string[]; c: number }[]; medio: { q: string; a: string[]; c: number }[]; dificil: { q: string; a: string[]; c: number }[] } }> = {
  1: { perguntas: { facil: [{ q: "O que é fundamental na Terapia Ocupacional?", a: ["Atividade Humana", "Apenas Medicamentos", "Isolamento"], c: 0 }], medio: [], dificil: [] } },
...
// Tentar importar o banco externo, se falhar ou não existir, usa o fallback de segurança
...
export default function App() {
  ...
  // Leituras protegidas contra undefined
...
const styles: Record<string, React.CSSProperties> = {
  ...
  card: { backgroundColor: '#15122C', borderRadius: '24px', padding: '30px', border: '4px solid #251F4F', boxShadow: '0
